...
class ProductBackendController extends yupe\components\controllers\BackController
{
    ...
    public function accessRules()
    {
        return [
            ['allow', 'roles' => ['admin'],],
            ['allow', 'actions' => ['index', 'ajaxSearch'], 'roles' => ['Multistore.ProductBackend.Index'],],
            ['allow', 'actions' => ['view'], 'roles' => ['Multistore.ProductBackend.View'],],
            ['allow', 'actions' => ['create', 'copy'], 'roles' => ['Multistore.ProductBackend.Create'],],
            [
                'allow',
                'actions' => ['update', 'inline', 'sortable', 'deleteImage'],
                'roles' => ['Multistore.ProductBackend.Update'],
            ],
            ['allow', 'actions' => ['delete', 'multiaction'], 'roles' => ['Multistore.ProductBackend.Delete'],],
            [
                'allow',
                'actions' => ['typeAttributes'],
                'roles' => ['Multistore.ProductBackend.Create', 'Multistore.ProductBackend.Update'],
            ],
            [
                'allow',
        ...
        if (Yii::app()->getRequest()->getIsPostRequest() && Yii::app()->getRequest()->getPost('Product')) {
            ...
            if ($model->saveData($attributes, $typeAttributes, $variants, $categories)) {
                ...
                Yii::app()->getUser()->setFlash(
                    ...
                    Yii::t('MultistoreModule.multistore', 'Record was added!')
                );
                ...
                Yii::app()->getUser()->setFlash(
                    ...
                    Yii::t('MultistoreModule.multistore', 'Failed to save product!')
                );
            }
        }
    ...
    public function actionUpdate($id)
    {
        ...
        if (Yii::app()->getRequest()->getIsPostRequest() && Yii::app()->getRequest()->getPost('Product')) {
            ...
            if ($model->saveData($attributes, $typeAttributes, $variants, $categories)) {
                ...
                Yii::app()->getUser()->setFlash(
                    yupe\widgets\YFlashMessages::SUCCESS_MESSAGE,
                    Yii::t('MultistoreModule.multistore', 'Record was updated!')
                );
    ...
    public function actionDelete($id)
    {
        if (Yii::app()->getRequest()->getIsPostRequest()) {
            ...
            Yii::app()->getUser()->setFlash(
                yupe\widgets\YFlashMessages::SUCCESS_MESSAGE,
                Yii::t('MultistoreModule.multistore', 'Record was removed!')
            );
            ...
            throw new CHttpException(400, Yii::t('MultistoreModule.multistore', 'Unknown request. Don\'t repeat it please!'));
        }
    }
        ...
        foreach ($model as $product) {
            $data[] = [
                ...
                'name' => $product->name . ($product->sku ? " ({$product->sku}) " : ' ') . $product->getPrice(
                    ) . ' ' . Yii::t('MultistoreModule.multistore', 'руб.'),
