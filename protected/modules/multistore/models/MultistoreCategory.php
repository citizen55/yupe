 ...
 *
 * @property-read MultistoreCategory $parent
 * @property-read MultistoreCategory[] $children
 *
 * @method MultistoreCategory published()
 * @method MultistoreCategory roots()
 * @method getImageUrl($width = 0, $height = 0, $options = [])
 *
 */
class MultistoreCategory extends \yupe\models\YModel
{
    ...
    {
        return '{{multistore_category}}';
    }

    /**
     * Returns the static model of the specified AR class.
     * @return MultistoreCategory the static model class
     */
    ...
    public function rules()
    {
        return [
            ...
            [
                ...
                'message' => Yii::t('MultistoreModule.multistore', 'Bad characters in {attribute} field')
            ],
            ['slug', 'unique'],
    ...
    public function behaviors()
    {
        ...
        return [
            ...
            'CategoryTreeBehavior' => [
                'class'                => 'multistore\components\behaviors\DCategoryTreeBehavior',
                'titleAttribute'       => 'name',
                'aliasAttribute'       => 'slug',
                'urlAttribute'         => 'url',
    ...
    {
        return [
            'parent'       => [self::BELONGS_TO, 'MultistoreCategory', 'parent_id'],
            'children'     => [self::HAS_MANY, 'MultistoreCategory', 'parent_id'],
            'productCount' => [self::STAT, 'Product', 'category_id'],
			// @mytodo добавиk связь на таблицу связь категория организация, пока не знаю будет работать или нет
			// возможно нужно будет сделать через through http://yiiframework.ru/doc/guide/ru/database.arr
			'org'		   => [self::HAS_MANY, 'CategoryOrg', 'category_id'],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'parent_id'         => Yii::t('MultistoreModule.multistore', 'Parent'),
            'name'              => Yii::t('MultistoreModule.multistore', 'Name'),
            ...
            'sort'              => Yii::t('MultistoreModule.multistore', 'Order'),
        ];
    }
    ...
    {
        return [
            self::STATUS_DRAFT     => Yii::t('MultistoreModule.multistore', 'Draft'),
            self::STATUS_PUBLISHED => Yii::t('MultistoreModule.multistore', 'Published')
        ];
    }
    ...
    {
        $categories = MultistoreCategory::model()->findAllByAttributes(['parent_id' => $parent_id], ['order' => 'name']);
    ...
    {
        if ($this->_url === null) {
            $this->_url = Yii::app()->getRequest()->baseUrl . '/multistore/' . $this->getPath() . Yii::app()->getUrlManager()->urlSuffix;
        }
