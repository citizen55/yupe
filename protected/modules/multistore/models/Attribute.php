 ...
 *
 * The followings are the available columns in table 'multistore_attribute':
 * @property integer $id
...
class Attribute extends \yupe\models\YModel
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            [
                ...
                'match',
                'pattern' => '/^([a-z0-9._-])+$/i',
                'message' => Yii::t('MultistoreModule.multistore', 'The name can contain only letters, numbers and underscores.')
            ],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'name' => Yii::t('MultistoreModule.attr', 'Name'),
            'title' => Yii::t('MultistoreModule.multistore', 'Title'),
            'type' => Yii::t('MultistoreModule.type', 'Type'),
            'required' => Yii::t('MultistoreModule.attr', 'Required'),
            'group_id' => Yii::t('MultistoreModule.attr', 'Group'),
            'unit' => Yii::t('MultistoreModule.attr', 'Unit'),
        ];
    }
    ...
    {
        return [
            self::TYPE_SHORT_TEXT => Yii::t('MultistoreModule.attr', 'Short text (up to 250 characters)'),
            self::TYPE_TEXT => Yii::t('MultistoreModule.attr', 'Text'),
            self::TYPE_DROPDOWN => Yii::t('MultistoreModule.attr', 'Dropdown list'),
            //self::TYPE_CHECKBOX_LIST => Yii::t('MultistoreModule.multistore', 'Список чекбоксов'),
            self::TYPE_CHECKBOX => Yii::t('MultistoreModule.attr', 'Checkbox'),
            //self::TYPE_IMAGE => Yii::t('MultistoreModule.multistore', 'Изображение'),
            self::TYPE_NUMBER => Yii::t('MultistoreModule.multistore', 'Число'),
        ];
    }
        ...
        switch ($this->type) {
            ...
            case self::TYPE_TEXT:
                return Yii::app()->getController()->widget(
                    Yii::app()->getModule('multistore')->getVisualEditor(),
                    [
                        'name' => $name,
                        'value' => $value
                    ],
    ...
    public function renderValue($value)
    {
        ...
        switch ($this->type) {
            ...
            case self::TYPE_DROPDOWN:
                ...
                break;
            case self::TYPE_CHECKBOX:
                $res = $value ? Yii::t("MultistoreModule.multistore", "Yes") : Yii::t("MultistoreModule.multistore", "No");
                break;
        }
    ...
    public function afterDelete()
    {
        ...
        $this->getDbConnection()->createCommand("DELETE FROM {{multistore_product_attribute_eav}} WHERE `attribute`='{$this->name}'")->execute();
